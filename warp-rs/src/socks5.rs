...
// 出网连接通过 tokio_smoltcp::Net 走 WireGuard 隧道的虚拟网卡，而不是宿主机
// 真实网卡，等价于 Go 版 wiresocks/proxy.go 里 tnet.Dial 的角色。domain ATYP
// 走 dns::resolve 在隧道内解析。
//
...
pub(crate) async fn handle_client(
    ...
    match req[1] {
        CMD_CONNECT => handle_connect(client, peer, req[3], net).await,
        CMD_UDP_ASSOCIATE => handle_udp_associate(client, peer, req[3], net).await,
        _ => unreachable!(),
    }
}
...
async fn handle_connect(
    ...
    let target = match read_target(&mut client, atyp, net).await {
        Ok(t) => t,
        Err(e) => {
    ...
    let outbound = match relay::connect(net, target.addr).await {
        Ok(s) => s,
        Err(e) => {
            let _ = reply(&mut client, REP_HOST_UNREACHABLE).await;
    ...
    println!("连接 {peer} -> {}: 已建立", target.display);
...
async fn resolve_udp_target(net: &Net, target: SocketAddrOrName) -> Result<(SocketAddr, String)> {
    match target {
        SocketAddrOrName::Addr(a) => Ok((a, a.to_string())),
        SocketAddrOrName::Name(name, port) => {
            let addrs = resolve(net, DNS_SERVER, &name, RecordType::A, DNS_TIMEOUT)
                .await
                .with_context(|| format!("隧道内解析域名 {name} 失败"))?;
            let ip = addrs
                .into_iter()
                .next()
                .ok_or_else(|| anyhow!("域名 {name} 未解析到地址"))?;
            let addr = SocketAddr::new(ip, port);
            Ok((addr, format!("{name}:{port}({addr})")))
        }
    }
}
...
async fn handle_udp_associate(
    ...
    let relay_sock = UdpSocket::bind(SocketAddr::new(bind_ip, 0))
        .await
        ...
        tokio::select! {
            ...
            r = relay_sock.recv_from(&mut relay_buf) => {
                ...
                if target_addr.is_none() {
                    if let Ok((addr, display)) = resolve_udp_target(net, target).await {
                        target_addr = Some(addr);
                        println!("连接 {peer} -> {display}: 已建立（UDP ASSOCIATE）");
                    }
                }
                if let Some(target) = target_addr {
                    let _ = tunnel_sock.send_to(&relay_buf[header_len..n], target).await;
                }
            }
            r = tunnel_sock.recv_from(&mut tunnel_buf) => {
                let (n, from) = r.context("隧道内 UDP 读失败")?;
                if target_addr != Some(from) {
                    continue; // 忽略非目的地址来源
                }
                if let Some(client_addr) = client_addr {
                    let mut wrapped = encode_udp_header(from);
                    wrapped.extend_from_slice(&tunnel_buf[..n]);
...
async fn read_target(client: &mut TcpStream, atyp: u8, net: &Net) -> Result<Target> {
    match atyp {
        0x01 => {
            let mut addr = [0u8; 4];
            ...
            let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::from(addr)), u16::from_be_bytes(port));
            Ok(Target {
                addr,
        ...
        0x04 => {
            ...
            let mut port = [0u8; 2];
            client.read_exact(&mut port).await?;
            let addr = SocketAddr::new(IpAddr::V6(Ipv6Addr::from(addr)), u16::from_be_bytes(port));
            Ok(Target {
                addr,
                display: addr.to_string(),
            })
